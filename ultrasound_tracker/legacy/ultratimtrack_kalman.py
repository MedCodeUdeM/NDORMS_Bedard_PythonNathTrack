...
class UltraTimTrackGeometricKalman:
    ...
    def __init__(self, config: Optional[UltraTimTrackKalmanConfig] = None):
        ...
        self.last_update_indices: Optional[np.ndarray] = None
    ...
    def update(self, timtrack_state: Optional[np.ndarray]) -> np.ndarray:
        ...
        self.last_update_indices = indices.copy()
        self.last_posterior_state = self.x.copy()
    ...
    def get_last_gain_diagonal(self) -> Optional[np.ndarray]:
        ...
        If the last update used all 4 states, this returns np.diag(K). If the
        last update was partial, returns a length-4 array with NaNs for states
        ...
        if self.last_update_indices is None:
            rows = min(K.shape[0], K.shape[1])
            for i in range(rows):
                gain_diag[i] = K[i, i]
            return gain_diag

        for col, idx in enumerate(self.last_update_indices):
            gain_diag[int(idx)] = K[int(idx), col]
